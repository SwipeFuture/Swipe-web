...
const BALANCE_COLOR = "#D97706"; // amber-600 — the hero series
const CONTRIBUTIONS_COLOR = "#A8A29E"; // stone-400 — the muted reference series
...
export default function GrowthChart({ years, currency }: { years: YearPoint[]; currency: Currency }) {
  ...
  const { ticks, niceMax, balancePoints, contributionPoints, xScale } = useMemo(() => {
    ...
    const balancePoints = years.map((y) => ({ x: xScale(y.year), y: yScale(y.endingBalance) }));
    const contributionPoints = years.map((y) => ({ x: xScale(y.year), y: yScale(y.totalInvested) }));

    return { ticks, niceMax, balancePoints, contributionPoints, xScale };
  }, [years, durationYears]);

  const baselineY = MARGIN.top + PLOT_H;
  const balanceLine = buildLinePath(balancePoints);
  const contributionLine = buildLinePath(contributionPoints);
  const balanceArea = `${balanceLine} L${balancePoints[balancePoints.length - 1].x.toFixed(2)},${baselineY} L${balancePoints[0].x.toFixed(2)},${baselineY} Z`;
    ...
    <div className="relative">
      ...
      <div className="relative overflow-hidden rounded-[32px] sm:rounded-[36px] border border-white/70 bg-white/60 backdrop-blur-xl p-6 sm:p-9 lg:p-10 shadow-xl">
        ...
        <div className="relative flex items-center justify-between flex-wrap gap-3">
          <h3 className="text-lg sm:text-xl font-bold tracking-tight text-stone-800">Balance Growth</h3>

          {/* Legend — line keys, not boxes */}
          <div className="flex items-center gap-4 text-xs sm:text-sm font-semibold text-stone-500">
            <span className="inline-flex items-center gap-2">
              <span className="inline-block h-[2px] w-4 rounded-full" style={{ backgroundColor: BALANCE_COLOR }} />
              Balance
            </span>
            <span className="inline-flex items-center gap-2">
              <span className="inline-block h-[2px] w-4 rounded-full" style={{ backgroundColor: CONTRIBUTIONS_COLOR }} />
              Contributions
            </span>
          </div>
        </div>
          ...
          >
            <defs>
              <linearGradient id="balanceAreaFill" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor={BALANCE_COLOR} stopOpacity={0.1} />
            ...
            <path d={balanceArea} fill="url(#balanceAreaFill)" stroke="none" />

            {/* Contributions — muted reference line */}
            <path
              d={contributionLine}
              fill="none"
            ...
            <circle cx={balancePoints[balancePoints.length - 1].x} cy={balancePoints[balancePoints.length - 1].y} r={5} fill={BALANCE_COLOR} stroke="#fff" strokeWidth={2} />
            <circle cx={contributionPoints[contributionPoints.length - 1].x} cy={contributionPoints[contributionPoints.length - 1].y} r={5} fill={CONTRIBUTIONS_COLOR} stroke="#fff" strokeWidth={2} />

            {/* Direct end-label on the hero series */}
            <text
              x={balancePoints[balancePoints.length - 1].x - 8}
              y={balancePoints[balancePoints.length - 1].y - 12}
              textAnchor="end"
              fontSize={13}
              fontWeight={700}
            ...
            {hovered && (
              <g>
                <line
                  ...
                  strokeWidth={1}
                />
                <circle cx={xScale(hovered.year)} cy={balancePoints[hovered.year].y} r={6} fill={BALANCE_COLOR} stroke="#fff" strokeWidth={2} />
                <circle cx={xScale(hovered.year)} cy={contributionPoints[hovered.year].y} r={6} fill={CONTRIBUTIONS_COLOR} stroke="#fff" strokeWidth={2} />
              </g>
            )}
          </svg>
          ...
          {hovered && (
            ...
            >
              ...
              <div className="mt-1.5 flex items-center gap-2 text-sm">
                <span className="inline-block h-[2px] w-3 rounded-full" style={{ backgroundColor: BALANCE_COLOR }} />
              ...
              <div className="mt-1 flex items-center gap-2 text-sm">
                <span className="inline-block h-[2px] w-3 rounded-full" style={{ backgroundColor: CONTRIBUTIONS_COLOR }} />
