...
import type { CompoundInputs, CompoundingFrequency } from "./calculations";
import type { Currency } from "../shared/currency";
import CurrencySelect from "../shared/CurrencySelect";
import ToggleSwitch from "../shared/ToggleSwitch";
import FrequencySelect from "./FrequencySelect";

const RATE_CHIPS = [2, 3, 5, 7, 8, 10, 12, 15];
...
export default function InputsCard({
  ...
  onChange: (patch: Partial<CompoundInputs>) => void;
  currency: Currency;
        ...
        <div className="relative grid sm:grid-cols-2 gap-6 sm:gap-7">
          ...
          <div className="sm:col-span-2">
            <FrequencySelect
              ...
              onChange={(f: CompoundingFrequency) => onChange({ contributionFrequency: f })}
            />
          </div>

          {/* 5. Annual Interest Rate */}
          <div className="sm:col-span-2">
            <span className="text-xs sm:text-sm font-semibold text-stone-600">Annual Interest Rate (%)</span>
            <div className="mt-2.5 flex flex-wrap gap-2">
              {RATE_CHIPS.map((pct) => (
                <Chip
                  key={pct}
                  label={`${pct}%`}
                  active={inputs.annualRatePct === pct}
                  onClick={() => onChange({ annualRatePct: pct })}
                />
              ))}
            </div>
            <div className="mt-3">
          ...
          <div className="sm:col-span-2">
            <FrequencySelect
              ...
              onChange={(f: CompoundingFrequency) => onChange({ compoundingFrequency: f })}
            />
          </div>
          ...
          {/* 9. Optional: Annual Contribution Increase */}
          <div>
            <div className="flex items-center justify-between">
